...
// (FedEx OnSite) to the customer's home. Password protected.
//
...
//   APP_PASSWORD         - password to open the site (if blank, no password)
//   APP_USER             - username, defaults to "ncg"
...
const APP_USER = process.env.APP_USER || "ncg";
...
// ---------------------------------------------------------------------------
// Password gate (HTTP Basic Auth). If APP_PASSWORD is blank, the site is open.
// ---------------------------------------------------------------------------
...
function auth(req, res, next) {
  ...
  if (scheme === "Basic" && encoded) {
    ...
    if (safeEqual(u, APP_USER) && safeEqual(p, APP_PASSWORD)) return next();
  }
  res.set("WWW-Authenticate", 'Basic realm="GOOMBA Append"');
...
async function wpPerson(params, maxRetries = 4) {
  ...
  while (true) {
    ...
    if (res.status === 200) {
      ...
      // The live API wraps matches in { results: [...], metadata: {...} }.
      // Older example docs showed a bare array, so handle both shapes.
      let data = [];
...
async function placePhone(placeId) {
  if (!placeId) return "";
  const url = GMAPS_BASE + "/place/details/json?place_id=" + encodeURIComponent(placeId) +
    "&fields=formatted_phone_number&key=" + encodeURIComponent(GMAPS_KEY);
  const { data } = await gFetch(url);
...
async function nearestWalgreens(lat, lng, homeAddr, limit = 3) {
  ...
  for (const r of results) {
    ...
    // Nearby search does not include phone numbers, so fetch it per store.
...
// ---------------------------------------------------------------------------
// Routes (everything behind the password gate)
// ---------------------------------------------------------------------------
...
// Debug: hit Whitepages with raw query params and return the untouched response
// so we can see exactly what comes back. The key travels in a header, not the
// URL, so the request shown here is safe.
//   /api/raw?name=John Smith&state_code=AZ
app.get("/api/raw", async (req, res) => {
  const fields = ["name", "first_name", "last_name", "street", "city", "state_code", "zipcode", "phone", "include_historical_locations", "include_fuzzy_matching"];
  const qs = new URLSearchParams();
  for (const f of fields) {
    const v = req.query[f];
    if (v != null && String(v).trim()) qs.append(f, String(v).trim());
  }
  if ([...qs.keys()].length === 0) {
    return res.json({ note: "Add query params, e.g. /api/raw?name=John Smith&state_code=AZ" });
  }
  const url = WP_BASE + "?" + qs.toString();
  ...
  try {
    ...
    try { out.body = JSON.parse(text); } catch (e) { out.body = text; }
  } catch (e) {
    out.error = String(e);
  }
  res.json(out);
...
app.post("/api/search", async (req, res) => {
  ...
  // Radius turns a city or zip into a center point and catches nearby towns,
  // so a Delray Beach search at 25 miles still finds someone in Boynton Beach.
  const radiusMi = Math.min(100, Math.max(0, parseInt(b.radius || 0, 10)));
  const hasCenter = !!(q.street || q.city || q.zipcode);
  if (radiusMi > 0 && hasCenter) q.radius = radiusMi;
  ...
  // Safety net: exact search found nothing but a location was given. Retry on
  // name plus state only, so a wrong or too-narrow city does not hide a match.
  if (data.length === 0 && hasCenter) {
    const q2 = {};
    ...
    if (b.phone) q2.phone = String(b.phone).replace(/\D/g, "");
  ...
  // Next of kin: walk the relatives in order, look each up, and stop at the
  // first one that has a phone number. That is the contact we want.
  let nok = { found: false, tried: 0 };
  if (b.trace_relatives !== false) {
    const region = regionFor(b, person);
    const rels = (person.relatives || []).slice(0, 8);
    for (const rel of rels) {
      ...
      if (rr.ok && rr.data && rr.data.length) {
        ...
        if (bp && bp.number) {
          nok = {
            found: true,
            name: noDashes(rel.name || rm.name),
            phone: noDashes(bp.number),
            type: noDashes(bp.type || ""),
            score: bp.score != null ? bp.score : null,
            tried: nok.tried,
          };
          break;
        }
  ...
  if (b.find_walgreens !== false) {
    const homeAddr = pv.best_address || [b.street, b.city, b.state_code, b.zipcode].filter(Boolean).join(", ");
    walgreens = await findWalgreens(homeAddr);
  }

  res.json({ status: "matched", widened, person: pv, nok, walgreens });
});

app.listen(PORT, () => {
  console.log("GOOMBA Append running on port " + PORT);
  console.log("Whitepages key: " + (API_KEY ? "yes" : "no") + " | Maps key: " + (GMAPS_KEY ? "yes" : "no") + " | Password: " + (APP_PASSWORD ? "on" : "off"));
});
